...
class TestDependencyGraphGenerator:
    ...
    def test_analyze_project(self, sample_project):
        ...
        assert graph.has_edge(
            "sample_pkg.module_b.ClassB", "sample_pkg.module_a.ClassA"
        )
...
class TestAnalyzeCodebase:
    ...
    def test_analyze_codebase_integration(
        self,
        sample_project,
        ...
        with patch("ctxclip.graph.DependencyGraphGenerator") as mock_generator, \
            patch("ctxclip.graph.merge_duplicate_nodes", return_value=graph):
            mock_instance = mock_generator.return_value
            mock_instance.analyze_project.return_value = graph

            # Call the function under test
            result_graph, _ = analyze_codebase(sample_project)
            ...
            if "sample_pkg.module_a.ClassA" in result_graph.nodes:
                assert (
                    result_graph.nodes["sample_pkg.module_a.ClassA"].get(
                        "is_public_api"
                    )
                    is True
                )
    ...
    def test_analyze_codebase_error_handling(
        self, sample_project, mock_api_extractor, mock_find_package_files
    ):
        ...
        with patch("ctxclip.expand.parse_file") as mock_parse:
            ...
            with patch("ctxclip.graph.DependencyGraphGenerator") as mock_generator:
                ...
                analyze_codebase(sample_project)
